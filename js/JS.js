...
*返回值：无
 */
Element.prototype.mytab=function(objtab){
//objtab是一个对象参数，可以为option添加任何属性，这样避免了漏传参数的情况，也避免了传参时顺序的不同
	if (objtab.tabeve==undefined) {
			objtab.tabeve="mouseover";
		}
		if (objtab.tabact==undefined) {
			objtab.tabact="act";
		}
		var _ulchild=this.childNodes;
		var _conchild=objtab.tabcon.childNodes;
		this.addHandler(objtab.tabeve,function(e) {
		...
		function showtab(_now){
			for (var i = 0; i < _ulchild.length; i++) {
				if (i==_now) {
					_ulchild[_now].addClass(objtab.tabact);
			        _conchild[_now].delClass("none");    
				}
				else{
					_ulchild[i].delClass(objtab.tabact);
...
/**实现选项卡切换的效果2*/
function mytab(_ul,_con,etype,actname){
	/*	etype=类型*/
		var _ulchild=_ul.childNodes;
		var _conchild=_con.childNodes;
		_ul.addHandler(etype,function(e) {
		var e=e||window.event;
        var target=e.target||e.srcElement;
        if (target.tagName=="LI") {
        	showtab(target.index());
		...
		function showtab(_now){
			for (var i = 0; i < _ulchild.length; i++) {
				if (i==_now) {
					_ulchild[_now].addClass(actname);
			        _conchild[_now].delClass("none");
			        /*addClass和delClass是调用JS.js里删除和添加一个样式*/
				}
				else{
					_ulchild[i].delClass(actname);
			        _conchild[i].addClass("none");
				}
			}
